---
title: "01 - Review of Basic Statistics using R"
...
categories: [introduction, review, econ 326, data, descriptive statistics]
format: 
  html: default
  ipynb:
    jupyter:
...
```{.callout-warning}
Key learning objectives should be:

* Load data
* Compute summary statistics
* Compute basic summary statistics table
* Do a t-test
...
## Part 1: Review of Data in R and Jupyter
...
You can imagine this as a spreadsheet in a program like Excel.  This is the way most statistical programs like to receive data - but it _isn't_ a property of the data itself.  It is only a representation; there can be other kinds of representations.  

> For example, a panel data set might have each row as a unit (e.g. country) and each column might represent a variable in a year (e.g. unemployment in 2016, unemployment in 2017, etc.).  Often, when we work with real-world data we have to reshape it in order for it to be usable.

Data can be stored in many different file formats (which encode how information is stored), which must be interpreted by our statistical programs. For example, data files with a `.dta` are formatted for STATA,  data files with a `.csv` suffix are formatted as comma-separated values, and data files with an `.Rda` suffix are formatted for R. This is referred to as **importing** data. There are different methods for importing data in different formats; for example:
...
* `read.csv()`: Importing data from a delimited text file: this is data which is in a text format, but where the columns are *delimited* or separated by a special character.  For example, the `.csv` (comma-separated variable) format is text, but where the columns are separated by a comma.
...
answer00 <- glimpse ??? #fill in the code here.
...
#finally, we can rename columns, to be more meaningful names, this is not necessary but is generally a good practice
...
You will notice that the code above looks a little different and uses the `%>%` operation.  This is called the **pipe operator**.  It is shorthand for "insert whatever comes before into the first argument of what comes after" - kind of like a series of pipes, hence the name.  You can think of it as saying "and then do..."
...
* This means that for **quantitative** variables, we need to make sure they are numeric
* And for **qualitative** variables that they are _factors_
...
SFS_data <- SFS_data %>%
            ...
            mutate(education = as_factor(education)) %>% #education is now a factor
