...
Do you recall how to make these plots from [Module 8](https://comet.arts.ubc.ca/docs/Research/econ490-r/08_ggplot_graphs.html)?
...
Notice that the diff-in-diff linear model in Equation (1) can be seen as a specific case of a linear model with many fixed-effects using panel data. We can still use the `plm()` function that we have studied in [Module 14](https://comet.arts.ubc.ca/docs/Research/econ490-r/14_Panel_Data.html) Remember to add the option `effect = "twoways"` to tell R to add both time and worker fixed-effects to the specification.
...
We can easily do that using the library `coefplot`, which we covered in [Module 8](https://comet.arts.ubc.ca/docs/Research/econ490-r/08_ggplot_graphs.html). We use the function `coefplot` from the same library and the coefficients we have saved in object *did_model* as inputs.
