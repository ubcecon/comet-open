...
capture noisily by year: generate var_one = 1 
```
...
capture noisily collapse (mean) avg_earnings = earnings (sd) sd_earnings = earnings (count) tot_emp = earnings [pweight = sample_weight], by(region year)
```
...
capture drop var
capture noisily by sex: generate var = _n
```
...
capture noisily reshape wide earnings sex, i(year) j(workerid)
```

```{python}
%%stata

capture noisily reshape error
```
