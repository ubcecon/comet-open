...
This is our dataset which we will be using for the first section of this notebook. Please run the cell below and see if the head command correctly displays the data. 
```{r}
...
Here we will make a basic summary table with  the varaibles Age, BMI, Weight, Race1, Education, Gender which will show a basic 
overview of the dataset along with this we will have first mean and then in brackets standard deviation and for specific 
...
## Part 2: Example usage for econ 326, 490

Finally, let's take a look at how we can use summary statistics tables for the applied sections of Econ 490 and Econ 326. We'll be using the **cancesus dataset, which requires an API key**. You can get an api key [here](). 
