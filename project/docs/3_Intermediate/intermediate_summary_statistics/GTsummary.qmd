...
author: 'COMET Team <br> __Irene Berezin, Alex Ronczewski__'
date: 1 March 2025
...
notebook. We have chosen this dataset as it is available within R as a
...
Here we will make a basic summary table with the variables
...
variables. Here the key function is `by` where by setting it to the
variable Diabetes we can show the summary statistics for three different
groups Overall, No Diabetes and Diagnosed with Diabetes.

We can also use the command `add_` to add certain statistics to our
...
[`tbl_summary()` tutorial](https://www.danieldsjoberg.com/gtsummary/articles/tbl_summary.html) under
...
$$
\texttt{TotChol}_i = \beta_0 + \beta_1 \cdot \texttt{Age}_i + \beta_2 \cdot \texttt{BMI}_i + \beta_3 \cdot \texttt{Alcohol12PlusYr}_i + \beta_4 \cdot \texttt{PhysActive}_i + \beta_5 \cdot \texttt{SleepHrsNight}_i + \varepsilon
$$

Here we will make a simple table showing our regression results, with
our beta coefficients, 95% confidence intervals, and $p$-values.
...
vectors we picked for our analysis from the cancensus API are indeed the
correct ones:
...
summary statistic table. We’ll begin by some minor data wrangling:
