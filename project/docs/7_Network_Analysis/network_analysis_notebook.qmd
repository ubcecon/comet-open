...
Instead of creating a graph with predetermined positions of nodes and edges we can also generate a random graph with a set amount of nodes and edges. Below you can change the amount of nodes and edges by changing num_nodes and num_edges. To do this we define a function called `generate_random_graph` which takes two parameters `num_nodes` and `num_edges` which correspond to the number of nodes and edges we wish to have in our new graph. 
...
# Values can of nodes and edges can be changed here
num_nodes = 5
num_edges = 12
...
position = nx.spring_layout(random_graph, seed=42)
nx.draw(random_graph, pos=position, with_labels=True, node_color='lightblue', 
        node_size=500, font_size=12, font_weight='bold')
edge_labels = nx.get_edge_attributes(random_graph, 'weight')
nx.draw_networkx_edge_labels(random_graph, position, edge_labels=edge_labels)
...
If we color the nodes of our aerian sea pottery network by eccentricity, we see an interesting result:
