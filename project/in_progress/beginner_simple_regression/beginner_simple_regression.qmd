...
In this notebook, we will explore these questions using our census data set and will learn more about the immigrant wage gap. If you remember from last lecture, we were interested in the relationship between `wages` and `immstat` - wages and immigration status. However we can also use to the variable, `mrkinc` (market income) to measure income.

Let's explore the relationship between `wages` and `mrkinc` in this data set. Let's start off with a visualization:
...
The expectation of $X$ is what outcome we expect $X$ to *typically* be after *a lot* of sampling. We calculate the predicted value by multiplying the different values $X$ can take by the various probabilities of $X$ taking on that value. For instance, the expectation of a die throw is 3.5. Essentially, 
$$
...
This means that *given* a particular value of years of education ($years_i$) for an individual $i$, the wages of that individual will follow the linear regression form $\beta_0 +\beta_1X$.
...
1.  Accurately estimate this equation (and especially the model parameters)
...
While this may look complicated, it is just the calculus way of writing "choose $\beta_0$ and $\beta_1$ ( $\hat{\beta_0},\hat{\beta_1}$) such that they minimize the **sum of the squared residuals (SSE)**".
...
- Regression models look like: `Y ~ X` where `Y` is regressed on `X` and the `~` symbol is called "tilde".
...
Which can be written in R as `mrkinc ~ wages`.
...
- The %-of the data explained by the model (R-squared)

### Test Your Knowledge

What % of the data is explained by the model? Answer to 2 decimal places.
...
Previously, we looked at the relationship between market income and wages. However, these are both *quantitative* variables. What if we wanted to work with a *qualitative* variable like `immstat`?
...
2.  The coefficients of the dummy variable have meaning in terms of the regression model
...
What is the difference in average wage between immigrants and non-immigrants?
...
The number above might seem familiar, if you remember what we learned about a $t$-test from earlier notebooks. Remember this result?
...
Tested objects: `answer_3` (the regression for males), `answer_4` (the regression for females).
...
answer_3 <- lm(... ~ ..., data = filter(census_data, ... == ...)) # what should replace the ...
# Hint: Don't forget the quotation marks when specifying the subset 

# Regression for females
answer_4 <-  ... # what should replace the ...

summary(answer_3) # Allow us to view the regression coefficient estimates
summary(answer_4) # Same as above, but for the female regression
...
answer_3 <- lm(wages ~ immstat, data = filter(census_data, sex == "male")) # what should replace the ...

answer_4 <-  lm(wages ~ immstat, data = filter(census_data, sex == "female")) # what should replace the ...
...
How do we interpret the intercept estimate on each of these regressions?
...
Compare the gaps. Is the immigrant wage gap larger for males or females? Why do you think that might that be?
...
Tested objects: `answer_7` (20 to 24 years), `answer_8` (50 to 54 years)
...
reg5_30 <- lm(wages ~ immstat, data = filter(census_data, agegrp == '30 to 34 years')) 
...
What happens to the immigrant wage gap as we move across age groups? What do you think might explain these changes?
...
Tested objects: `answer_10` (regression without controls), `answer_11` (regression with controls).
...
reg2A <- lm(wages ~ immstat , data = census_data) 

reg2B <- lm(wages ~ immstat + hdgree, data = census_data)
...
Compare the estimated immigrant wage gap with and without $E_i$ in the regression. What happens to the gap when we add $E_i$? How do we interpret this?
...
Test objects: `answer_13` race regression.
...
reg_race <- lm(wages ~ vismin, data = census_data) 
...
How should we interpret the regression estimate for `visminblack`?
...
With this racial wage gap in mind, should we add explanatory variables for race to our regression from activity 2 and 3? Why or why not?
