---
title: "2.6 - Intermediate - Conditional Expectations (398)"
...
categories: [intermediate, econ 398, regression, R, conditional expectations, conditional varainces, confidence intervals, comparison measure, summary statistics, t-test]
format: 
  html: default
  ipynb:
    jupyter:
...
<<<<<<<< HEAD:Intermediate/intermediate_conditional_expectations_398/intermediate_conditional_expectations_398.qmd
### References
========
...
>>>>>>>> Avi_consolidating_notebooks:notebook_recycling_bin_plz_use_me/intermediate_conditional_expectations(398)/intermediate_conditional_expectations(398).qmd
...
## Part 1: Conditional Expectations with Simulated Data

Because this series of notebooks cover techniques used to recover causal effects, many of which we can never know with certainty in real-world data, we will use synthetic data to model the true data generating process.

To ensure replicable results when working with random synthetic data, one needs to set the seed for the [pseudorandom number generator](https://en.wikipedia.org/wiki/Pseudorandom_number_generator). With a specified seed, the code will always produce the same data set. 
========
...
In the following code we simulate a data set of $5000$ observations that resembles an earnings data set. Let's begin with four basic variables: gender, age, years of education, and tenure. The values used for the various functions are taken from an analysis of a real data set to ensure that the simulated data still looks sufficiently realistic.
...
<<<<<<<< HEAD:Intermediate/intermediate_conditional_expectations_398/intermediate_conditional_expectations_398.qmd
To make the data generating process more realistic, we shift the data by the desired means, round the entries, and bound the variables to the subset of interest. 
========
...
Here we specify the "true" data generating process for log-hourly wages.
...
The following code let's us see the summary statistics of our simulated dataset. We will discuss summary statistics in more detail towards the end of this notebook.
========
...
where $j$ subscripts observations instead of unique values of $X$. Appendix 2 explains why the average is a good estimator for the expectation of a variable. 

In `R`, the function `mean` computes the average for you. You can also do it manually using a combination of `sum` and `length`. A third way is using `lm`. Intuitively, we can rewrite each data point as the sum of the mean and its deviation
...
<<<<<<<< HEAD:Intermediate/intermediate_conditional_expectations_398/intermediate_conditional_expectations_398.qmd
We can then replace $\mathbb{E}[X]$ with any arbitrary variable name (e.g., $\alpha$) and treat this as an estimation problem of computing a value for $\alpha$ that minimizes some score function of the errors $e_{i}$. `lm(X ~ 1, data = df)` estimates this model for a specific assumption about the score function. We then retrieve the estimate of the mean by calling the `coefficients` property of the resulting object.
========
...
<<<<<<<< HEAD:Intermediate/intermediate_conditional_expectations_398/intermediate_conditional_expectations_398.qmd
The extension from expectations and the average to conditional expectations is straightforward because [conditional expectations](https://en.wikipedia.org/wiki/Conditional_expectation) are the expected values of a variable given specific values of other variables. Mathematically, the conditional expectation is the following object,
$$
\begin{align}
...
Notice that the object that results is a function of the conditioning variable, $Z$. Unlike the expectation, which is a fixed value, the conditional expectation of a variable is a function of some other variable. If this seems confusing, approach this by first imagining $Z$ to be a discrete variable. 

For example, suppose $X$ is years of completed education and $Z$ is gender. Then it hopefully makes sense that

$$
    m(\text{Gender}) = \mathbb{E}[\text{Years of Completed Education} \mid \text{Gender}] 
$$

is a function while

$$
    m(\text{Gender} = \text{Female}) = \mathbb{E}[\text{Years of Completed Education} \mid \text{Gender} = \text{Female}] 
$$
...
For example, the estimated conditional mean of the years of completed education among females is the average of the years of completed education among females. Note that unlike the unconditional mean, showing that this conditional average is the appropriate estimator for the conditional mean is not straightforward even though it seems intuitively correct. For more on this refer to Appendix 2.

In `R`, we get the subset of data matching the condition using the syntax `X[Z==z]`. If your data is loaded in a `tibble`, we can also use the syntax `df %>% filter(Z==z) %>% summarise(mean = mean(X)) %>% .[['mean']]`. The second syntax involves less copy and pasting when you wish to change your variable names later. 

The `lm` version of the `tibble` syntax is `sim.data %>% lm(X ~ Z - 1, data = .) %>% .[['coefficients']] %>% .[['Zz']]`. For now all you need to know about what `lm(X ~ Z - 1, data = .)` does is that it is estimating the values of $\alpha_{j}$ that minimise some score function of the errors $e_{i}$ in
...
if $X$ is discrete. The variance describes the dispersion of the data around the mean; it is the mean of the squared distance between each data point and the mean. The greater this value, the greater the dispersion. If all data is concentrated around the mean, the variance will be near zero. As the shape of the distribution diverges from a "bell" shape, the variance increases. At the extreme, if the distribution of your data is flat or even concentrated at two poles far from the mean (i.e., a [bimodal distribution](https://en.wikipedia.org/wiki/Multimodal_distribution)), then your variable will have very high variance. The empirical estimator of the variance is
...
<<<<<<<< HEAD:Intermediate/intermediate_conditional_expectations_398/intermediate_conditional_expectations_398.qmd
To see why this correction is needed, refer to Appendix 2.
========
...
<<<<<<<< HEAD:Intermediate/intermediate_conditional_expectations_398/intermediate_conditional_expectations_398.qmd
If it is not clear why this is not an unbiased estimator of the true standard deviation, recall that due to [Jensen's Inequality](https://en.wikipedia.org/wiki/Jensen%27s_inequality), in general for a non-linear function $f$ we will have that $\mathbb{E}[f(X)] \ne f(\mathbb{E}[X])$. However the standard deviation is a case when there is [no single straightforward solution for the unbiased estimator](https://en.wikipedia.org/wiki/Unbiased_estimation_of_standard_deviation) and therefore we simply accept that we use a biased estimator. The definition of the conditional versions follow similarly by replacing the variance with the conditional variance and the variance estimator with the conditional variance estimator.
========
...
<<<<<<<< HEAD:Intermediate/intermediate_conditional_expectations_398/intermediate_conditional_expectations_398.qmd
The proof that this is the unbiased estimator follows exactly the same steps as for the variance estimator in Appendix 2. The conditional covariance replaces the expectations in the expression with the respective conditional expectations. The empirical estimator of the conditional covariance is adjusted similarly from the covariance estimator, by restricting the estimation sample to the subsample that satisfy the condition. We will not cover why now, but conditional covariances are a key object you will need to become familiar with for more advanced econometrics applications.
========
...
We will largely gloss over the detailed theory now, and focus on how to run these two tests in practice.
...
For our purposes, you only need to know this to interpret your results properly. The theory behind this is beyond the scope of this notebook.
...
## Part 4: Describing Distributions More Generally
...
<<<<<<<< HEAD:Intermediate/intermediate_conditional_expectations_398/intermediate_conditional_expectations_398.qmd
For example, let's randomly set some genders to missing to demonstrate how `model.matrix` handles this.
========
...
<<<<<<<< HEAD:Intermediate/intermediate_conditional_expectations_398/intermediate_conditional_expectations_398.qmd
## Appendix: Summary statistics for research projects
========
...
Because we would like a written report and in general the text-format output of `stargazer` does not look as nice in a report as it does in a notebook, we should report results in a slightly different way.
...
1.  In describing the summary statistics, point the reader to specific moments of interest. For example, it might be of interest if say the average age of the respondent in your data is particularly young or particularly old. *Et cetera*.
