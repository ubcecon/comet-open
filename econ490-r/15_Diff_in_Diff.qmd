...
Why are panel data sets crucial in diff-in-diff research designs? The idea is that panel data allows us to control for unobserved time invariant heterogeneity. 
...
Next, we plot the trend of average earnings by each group. Remember that we have seen how to make graphs in [Module 8](https://comet.arts.ubc.ca/docs/econ_490/econ490-r/8_ggplot_graphs.html).
...
This confirms that our parallel trend assumption is supported by the data. In other words, there are no observable differences in trends prior to the enactment of the training program. 

Checking the p-value of those coefficients prior to the treatment is called the **pre-trend test** and does not require any fancy work. A mere look at the regression results suffices!
