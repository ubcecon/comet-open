...
The plot above shows the the event study for both doses type. In this case we chose to aggregate over different dose types, however if we had differing event times, we could aggregate over them too. The choice of what to aggregate over depends on the research question.
