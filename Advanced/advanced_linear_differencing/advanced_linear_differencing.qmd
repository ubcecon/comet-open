...
```{r}
set.seed(123)
fake_data_3 <- dgp(N=100, T=50, a=6, g=10)
fake_data_3$treatment <- 0
for (grp in 2:6) {
  fake_data_3$treatment <- ifelse((fake_data_3$group == grp & fake_data_3$time >= fake_data_3$jump_time), 1, fake_data_3$treatment)
}
...
```{r}
summary_3 <- bacon(var1 ~ treatment, data = fake_data_3, id_var = "id", time_var = "time")
print(summary_3)
```
...
```{r}
ATT_3 <- weighted.mean(summary_3$estimate, summary_3$weight)
print(ATT_3)
#Should be 42.12366
```
