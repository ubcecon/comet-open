---
title: "Introduction to Statistical Concepts: Econ 325 Prep Course"
...
categories: [introduction, econ intro, probability, data, descriptive statistics, visualizations, inferential statistics]
toc: true
format: 
  html: default
  ipynb:
...
* [Timbers, Campbell, and Lee - Chapter 10: Statistical Inference](https://datasciencebook.ca/inference.html)
...
![A standard six-sided die, used for a number of games (Credit: [Roger McLassus](https://commons.wikimedia.org/wiki/File:Dice_2005.jpg))](media/Dice_2005.jpg){fig-alt="game die"}

* Rolling the die would be considered the _trial_ in this example, which could result in six distinct _outcomes_ based on which side of the die is facing up: 1,2,3,4,5,6. 
* To  help differentiate the outcomes from other mathematical objects, we can use _probability notation_ and list all possible outcomes within curly brackets, separated by commas: {1,2,3,4,5,6}.
* This list of outcomes makes up the _sample space_. 
* We could also group our outcomes into _events_, such as 'rolling an even number' or 'rolling an odd number'.

We can think about probability in terms of _likelihood_, which means how "likely" an event is. If we are familiar with the circumstances surrounding an event (the trial, outcome(s), and sample space) we are able to understand the likelihood of an event. For each event, there is both a theoretical and experimental probability^[Sometimes also called the _empirical_ probability]. The theoretical probability can be calculated as follows:
...
For example, $P(\text{rolling an odd number})$ would be the number of outcomes in this event, which is three (rolling a one, a three, or a five), divided by the total possible outcomes which we know to be six. Therefore, $P(\text{rolling an odd number}) = 0.5$. 
...
So we now know how to calculate the probability that an event will occur. What about the probability that it will _not_ occur? This is called the **complementary event**. 
...
To better understand these ideas, we will use a case study: _salmon fishing_ in British Columbia.
...
![A sockeye salmon, one of BC's most distinctive fish (Credit: [Milton Love](https://commons.wikimedia.org/wiki/File:Sockeye_salmon_swimming_right.jpg))](media/Sockeye_salmon_swimming_right.jpg){fig-alt="sockeye salmon"}

![Chinese immigrant Joe Sum fishing for Salmon in Vancouver, BC, c. August 1945.  Many immigrants and Indigenous people worked in salmon fisheries and canneries during the 20th century. (Credit: [Library and Archives Canada](https://recherche-collection-search.bac-lac.gc.ca/eng/home/record?app=fonandcol&IdNumber=3601932))](media/Joe_Sum_salmon_fishing.jpg){fig-alt="Chinese immigrant Joe Sum fishing salmon"}

Imagine you’re out fishing for salmon. Each time you cast out your line, there are five possible types of salmon it may come up with: _Sockeye_, _Chum_, _Coho_, _Chinook_, and _Pink_. For the following questions, assume that each type of salmon is equally likely to bite your fishing rod. 
...
Despite the integral role of salmon in supporting BC's coastal ecosystems and communities, data show that many salmon populations are declining due to pressures from climate change, habitat loss and pollution, and fishing. As part of its response, the department of Fisheries and Oceans Canada restricts the [type and size of fish](https://www.pac.dfo-mpo.gc.ca/fm-gp/rec/bc-zones-cb-eng.html#management-areas) commercial fishers are allowed to catch and retain. Off the coast of Salt Spring Island, for example, it is not permitted to catch and take home any Coho, Chinook, or Sockeye salmon. 
...
In this case, $A$ and $B$ are **mutually exclusive events**, meaning that they have no common outcomes. In any of the possible outcomes of what salmon you catch, either event $A$ _or_ event $B$ will occur, but never both $A$ and $B$ at the same time. This is the case with all sets of events which are complementary events, as there are no common outcomes. 

![A chart displaying the outcomes included in events A and B.](media/diagrams/events_outcomes1.svg) {fig-alt="A chart displaying the outcomes included in event A (Chum, Pink) and event B(Chinook, Sockeye, Coho)"}

Suppose you’re allowed to take home Chum and Pink salmon, but you only enjoy eating Sockeye, Coho, and Pink salmon. We’ll consider catching a salmon you are allowed to take home as event $A$, and catching a salmon you _want_ to take home event $B$. These events are not mutually exclusive. They each have an individual probability of occurring, as well as a joint probability of both events occurring. 

![A chart displaying the outcomes included in events A and B.](media/diagrams/events_outcomes2.svg){fig-alt="A chart displaying the outcomes included in event A (Chum , Pink) and event B(Sockeye, Coho, Pink)"}
...
This intersection is the joint-probability, or the probability of both event $A$ _and_ $B$ occuring. The event $(A\cap B)$ occurs only when both $A$ and $B$ occur simultaneously. There is only an intersection when the events are not mutually exclusive.

![A venn diagram to help visualize the concept of joint-probability.](media/diagrams/joint_probability.svg){fig-alt="A venn diagram of circles A and B, which shows that the joint-probability of events A and B can be visualized as the area of overlap."}
...
Event $(A \cup B)$ occurs if either $A$ _or_ $B$ _or_ both occur. This can happen regardless of whether $A$ and $B$ are mutually exclusive.  

![A pair of venn diagrams to help visualize types of unions.](media/diagrams/types_of_unions.svg) {fig-alt="A pair of venn diagrams representing relationships between events A and B; mutually exclusive events have no area of overlap, whereas non-mutually exclusive events do"}
...
![A pair of venn diagrams to help visualize probability calculations.](media/diagrams/calculating_unions.svg) {fig-alt="A pair of venn diagrams visualizing the calculation of the probability for union."}
...
![A diagram of fish in a pond, showing how probability changes as the number of a certain type of fish decreases](media/diagrams/fishing_pond.svg) {fig-alt="A visual of a pond with ten salmon, and the resulting pond of nine salmon after one salmon as been removed."}

You can easily see now how removing a fish from the pond after you catch it would affect the probability of the next fish you catch. In this case, the succession of trials would _not_ be independent. 
...
![A contingency table showing a possible configuration of hatchery and wild salmon caught for two different species](media/diagrams/contingency_table.svg) {fig-alt="A contingency table; wild Coho = 32, hatchery Coho = 60, total Coho = 92, wild Sockeye = 15, hatchery Sockeye = 50, total Sockeye = 65, total salmon = 157"}
...
You are able to easily see that the probability of catching a wild salmon is $47/157 = 0.299$. Would this probability change if we knew that we were only going to catch Coho? You might notice that wild salmon make up a much larger percentage of the Coho, so we would have a higher probability of catching a wild salmon in the case where we know it will be a Coho salmon. This is because we're now observing a _conditional_ distribution of the sixty-five Coho salmon. This probability is expressed as:

$$
P(A|B)
$$

Where $A$ would be the event 'catching a wild salmon' and $B$ would be the event 'catching a Coho salmon'. $P(A|B)$ then means 'the probability of catching a wild salmon _given_ that it's a Coho salmon'. In this case $P(A|B) = 32/92 = 0.35$.
...
You may have noticed that the probability of an outcome had a lot to do with the _possibilities_ that existed for a trial. In determining probability, we often will need to know that total number of possible outcomes, and what those outcomes are. To do that, we need to have efficient ways to count them!
...
>_Fishing Weirs_: You may or may not be familiar with the term 'weir', which is a structure placed across a channel of water to trap fish. In BC, many Indigenous fishers used this technology to not only catch fish, but to collect information on the size of the year's salmon run, which could inform harvesting practices. Weirs were banned by the Canadian government in an effort of colonization, but in recent years, some coastal communities have been reviving the practice as an effective method to continue monitoring salmon populations.  

![A fishing weir from the mid-1800s on the Cowichan River (Credit: [Fredrick Dally](https://commons.wikimedia.org/wiki/File:Salmon_weir_at_Quamichan_Village_on_the_Cowichan_River,_Vancouver_Island.jpg))](media/Salmon_weir_at_Quamichan_Village.jpg){fig-alt="Fishing Weir"}
...
![A tree diagram of the possible ways to go on a fishing trip based on the above table.](media/diagrams/tree_diagram1.svg) {fig-alt="A tree diagram summarizing all 12 possible outcomes fishing trips from the list of options."}

When it's drawn in a tree format like this, we can simply count up the total number of outcomes. When we don't want to take the time to draw a diagram, or the numbers are too large to do so, we can also calculate the total outcomes using the **fundamental counting principle**. By this principle, we know that if there are _m_ choices at the first stage, and _n_ choices at the second stage, then the total number of outcomes is $m \cdot n$. 
...
![A table showing the possible outcomes for each "spot"](media/diagrams/possible_outcomes.svg) {fig-alt="A table showing the five possible options of species (Pink, Coho, Sockeye, Chinook, Chum) for each of the five fish caught"}

**Question 7:** Use the fundamental counting principle to calculate how many arrangements you can make using all five fish. Give your answer as a whole number, with no decimals. _Hint: the total number of "levels" you have is five, and each species of fish represents a possibility at each level._
```{r}
#| tags: [question]
#| eval: false
...
![A tree digram showing all the possible outcomes for a pond with a Pink, Coho, and Chum salmon"](media/diagrams/tree_diagram2.svg) {fig-alt="A tree diagram showing that there are sie possible outcomes from a pond with this combination of fish"}

When order is important to the arrangement of elements, it is called a **permutation**. When you're using _all_ the possible elements at a time, the number of permutations in a set of $n$ elements is equal to $n!$. We can see this in the above decision tree. If we were to only have the option of catching _Pink_, _Coho_, or _Chum_ salmon ($n = 3$), then our number of permutations would be:
$$
3! = 3 \times 2 \times 1 = 6 
$$  

Now, what if we're calculating how many permutations can be made for just a _portion_ of the available elements? For example, how many permutations of two different salmon can we create from a list of all five salmon? 
...
**Question 9:** How many permutations of _three_ different salmon can we create from a list of all five salmon?
```{r}
#| tags: [question]
#| eval: false
...
>_Note_: This equation still holds true when the number of total elements is the same as the number of elements in our permutation. $0!$ is equal to 1, not 0! When $n = r$, the bottom of the equation becomes one, so our number of permutations is just equal to $n!$, as we determined in previous examples. 
...
![A comparison of two ponds, displaying how the number of permutations changes when there are repetitions in the dataset"](media/diagrams/permutations_repetitions.svg) {fig-alt="A visualisation showing how the number of permutations was is reduced when a repeat of the element (two of the same salmon species) are introduced"}
...
**Question d:** How many permutations could we generate from the following list of elements: _Chum_, _Pink_, _Pink_, _Coho_, _Coho_, _Sockeye_? 
...
**Question 11:** Earlier we calculated the number of permutations of three different salmon we could make from a list of all five salmon. How many _combinations_ of three could we make from a list of all five salmon? 
```{r}
#| tags: [question]
#| eval: false
...
> 1. Are you concerned about the state of salmon stocks in BC  (yes or no)
> 2. How much salmon do you consume in a year _________ _(grams)_ 
...
This is because the data collected from these two questions are _fundamentally different types of data_, and therefore have fundamentally different ways of being reported, used, and interpreted.  
...
![A table of sample cross sectional data.](media/diagrams/cross_sectional_data.svg) {fig-alt="Sample data recording information about participants' age, height, city of residence, etc."}
...
> _Think deeper_: Can you calculate the means for these data? How does knowing both values of central tendency change how you understand the distribution of the data? 
...
* Point estimates and statisitcal inference
* Sampling vairability and normal distribution
...
If you are catching salmon for food, however, it might not be immediately clear what kind of limitation this places on how much salmon you have available to eat; someone eating salmon may not typically think about it in terms of length. They instead might want to think about this regulation as the _relationship_ between the salmon's length and its weight. 
...
Keep in mind though that this equation represents the "true" relationship that exists. Meanwhile, when we create models, we are _predicting_ the values as best we can based on our data. Therefore, we would write are linear models in the following format:
...
![A diagram which shows that the sample is a representation of the population.](media/diagrams/population_sample.svg) {fig-alt="A diagram displaying how sample data is taken from a larger populuation"}
...
Where $n$ is the number of 'successes' for the proportion of interest (in our case, how many people said that they _are_ concerned about BC's salmon stocks) and $x$ is the total number of observations in the dataset (how many people were surveyed).

> _Note_: When we talk, we would normally describe proportions using percentages. People frequently say things like "we've completed 60% of our tasks", or "12% of our clients were unsatisfied with their service", as this helps when communicating mathematical information verbally. When writing things formally in statistics, however, we use decimal points to convey the same information. For example, 60% becomes 0.6, and 12% becomes 0.12. 
...
![A table of sample data of the primary reason the polled participants feel concerned about salmon stocks.](media/diagrams/sample_1_data.svg.svg) {fig-alt="Not concerned = 13, ecosystem/environmental = 44, livelihood = 15, consumption = 28, total = 100"}
...
![Another table of sample data of the primary reason the polled participants feel concerned about salmon stocks.](media/diagrams/sample_2_data.svg) {fig-alt="For the polled participants, not concerned = 17, ecosystem/environmental = 47, livelihood = 23, consumption = 13, total = 100"}

**Question 21:** What is the proportion of _this_ sample who are concerned about salmon stocks for reasons related to ecosystem health? Please provide your answer as a decimal value (e.g. 50% = 0.5).
```{r}
#| tags: [question]
#| eval: false
...
Notice how the shape of the histogram changes? With this larger set of data, it looks far more symetrical, with a single peak close to 0.5. This bell-curve shaped is called a **normal distribution** of data. 
...
![An example of a sampled value which is very different from the majority of other samples](media/diagrams/histogram_100.jpg) {fig-alt="A bin in the histogram records a value much higher than most of the other values"}
...
This is because statistics is directly connected to probability. Statistical inference is based on understanding the _probability_ that with any given sample size, ($n$), the true characteristic of the population ($\sigma$ or $\mu$) will fall a certain distance from the sampled value ($p$ or $\bar{p}$).
