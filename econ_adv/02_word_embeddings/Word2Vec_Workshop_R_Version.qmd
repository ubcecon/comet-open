...
In general, CBOW is faster and does well with frequent words, while Skip-Gram potentially represents rare words better.
...
rm(cos_sim_words_rounded, cos_sim_words, cos_sim_array, text_corpus, dtm_df_novel)
```
...
## Embeddings 
Note: the output here is very different than the python version, even though the model is using the same parameters and same input, which is *sentences*
