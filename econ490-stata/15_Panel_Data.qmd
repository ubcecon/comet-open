...
categories: [econ 490, stata, panel data, regression, fixed effects, random effects, heteroskedasticity, serial correlation, causality]
format: 
  html: default
  ipynb:
      syntax-definitions: 
...
We learned the techniques to create a balanced panel in [Module 7](https://comet.arts.ubc.ca/docs/econ_490/econ490-stata/7_Within_Group.html). Essentially all that is needed is to create a new data set that includes only the years for which there are no missing values. 
...
### 15.3.2 What if We Want to Control for Multiple Fixed Effects? 
