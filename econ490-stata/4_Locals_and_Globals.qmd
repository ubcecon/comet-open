...
When we ran `summarize` above, Stata produced output that was stored in several local variables. We can access those stored results with the command `return list` (for regular commands) or `ereturn list` (for estimation commands, which we'll cover later in [Module 11](https://comet.arts.ubc.ca/docs/econ_490/econ490-stata/11_Linear_Reg.html)). Since `summarize` is not an estimation command, we can run the following:
...
Locals are automatically generated whenever we use loops (as discussed in [Module 3](https://comet.arts.ubc.ca/docs/econ_490/econ490-stata/3_Stata_Essentials.html). In this subsection, we will see how to use locals both **inside** the loop (these locals are automatically generated by Stata) and **outside** the loop (when we store the list of values to loop from into a local).
