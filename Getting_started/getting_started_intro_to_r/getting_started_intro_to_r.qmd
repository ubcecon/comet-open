...
my_vector <- c(10,11,12,13,14,15) # replace ... with the appropriate code

answer_1 <- my_vector[4] # replace ... with the appropriate code
...
2. Extract the elements of the second object, and store it in the object `answer_6`.
...
3. Now, select the third element from the column `col4` of the data frame `hidden_df` and store it in `answer_8`. Try to do that without actually seeing the dataset.
