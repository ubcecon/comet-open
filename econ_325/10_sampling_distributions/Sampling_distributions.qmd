...
Since collecting data from the entire population is often unrealistic, we can instead survey a **sample** and gather a **sample estimate.** From our example, it would be much more practical to survey a 1000 people in Canada and calculate their average height. While this sample estimate won't be a true match, it's probably a good estimate. But how do we know we picked a representative sample of the Canadian population?
...
1.  The graphs that our bootstrap sampling distribution visually serves as a good approximation of our sampling distribution. The standard error for the bootstrap sampling distribution (0.00402) is close to our sampling distribution standard error (0.0433).
