...
author: COMET Team <br> _Oliver (Junye) Xu, Anneke Dresselhuis, Jonathan Graves, Sarthak Kwatra_
date: "2023-06-01"
description: This notebook goes into detail about how data is spread out, and what that means for its interpretation. This includes both how individual values may vary, and how values may _co-vary_ with one another.
...
Measures of dispersion describe the spread of data, that is, the possible values that a variable in a data set can take on. Common measures of dispersion which we'll look at include the Range, Interquartile Range, Standard Deviation, and Variance. Let's start looking at Dispersion with Range and Interquartile Range. 

### Range and Interquartile Range

We've already seen the **range** and **interquartile range** in the _Central Tendency_ notebook. Remember, the **range** is the difference between the maximum and minimum value that a variable takes on, while the **interquartile range** is the difference between the 75th and 25th percentile values. Remember a percentile reflects the portion of values below a certain value. For instance, if the 75th percentile of a data set is 100, this means that 75% of all values in the data set lie below 100, and of course the remaining 25% of values in the data set are above 100. We can use functions like `quantile()` and `fivenum()` to calculate these statistics quite quickly. 
...
> To directly calculate the interquartile range in R, the function `IQR()` can be used. 

### Variance 

The **variance** is the average of the squared differences from the mean. This seems like a complicated formula, so it makes some sense to delve into the reasoning behind the formula. Intuitively, a neat way to measure the dispersion of a data set is the average distance of each observation to the mean of the data set. Let's say our data set only has values 98, 100, 102. The mean is 100. The distance between the other two points and the mean are respectively, -2 and 2. The average of these distances are 0, which would indicate that there is no dispersion in the data. To account for negative differences, we square the values to get positive values. A small variance indicates that observations tend to fall close to the mean, while a high variance indicates that observations are very spread out from the mean. The standard deviation (discussed next) is also the square root of variance, although we we won't spend much time on variance here. 
...
The biggest difference between the two is that the variance is a squared measure and does not have the _same units_ as the data in the way that the standard deviation does. For example if we have a data set consisting of the heights of a group of students, the mean could be 170cm. However, the variance would be in units $cm^2$, hence why we usually work with standard deviation instead.

### Standard Deviation

The most commonly referenced measure of dispersion is **standard deviation**. This statistic measures dispersion around the mean. More specifically, it takes the squared value of the sum of the squared difference between each value and the mean for a given variable. You can think of this as a way to calculate the average difference of each data point from the mean of the data set.  
...
Similar to representing population mean with $\mu$ and sample mean with $\overline{x}$, population standard deviation is represented by $\sigma$ and sample standard deviation by $s_{x}$.
...
```{r}
# calculate the variance of wage, filtering away all of the NAs as before
...
answer_6 <- sqrt(...(census_data$wages, na.rm = TRUE))
# note that once you know the function, you can also write the answer as ...(variance), since we already assigned variance <- var(census_data$wages, na.rm = TRUE)
...
**Covariance** is a measure of the direction of a relationship between two variables. More specifically, it can be broken down into a positive case (meaning the two variables are positively related) and a negative case (meaning the two variables are negatively related). If two variables are positively related, it means that when one goes up, we can expect the other to go up, and vice versa when one goes down, so does the other. If two variables are negatively related, it means that when one goes up, the other goes down and vice versa. It is similar to the idea of variance that we just covered, but where variance measures how a _single_ variable varies, covariance measures how _two_ variables vary together.
...
$$
cov_{x,y}=\int\int(x_{i}-\bar{x})(y_{i}-\bar{y})f(x,y)~dx~dy
$$
...
```{r}
# constructing our hypothetical dataset
x <- c(6, 8, 10)
y <- c(25, 100, 125)
```
...
> **Note**: Even though a covariance value or correlatation coefficient may have a value of 0, this does not mean that there is no relationship at all - this only means that there is no _linear_ relationship between the two variables at hand.
...
answer_1 <- round(mean(package_data), digits = 2) # enter your answer here for mean
answer_2 <- round(sd(package_data), digits = 2) # enter your answer here for standard deviation 
answer_3 <- round(var(package_data), digits = 2)# enter your answer here for variance
