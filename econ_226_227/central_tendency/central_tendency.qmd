---
title: "01 - Introduction to Central Tendency using R"
author: COMET Team <br> _Sarthak Kwatra_ 
version: 0.1
date: 2023-07-20
...
categories: [introduction, review, econ 226, econ 227, central tendency, mean, median, mode, skewness, dispersion]
format: 
  html: default
  ipynb:
    jupyter:
...
Statistics is an economist's arsenal of techniques and tools that allows them to extract meaningful insights from data, and this course establishes all the sound foundations that an economist needs. A lot of these tools calculate a single representative value that summarizes the data in one way or another. 
...
If you don't have any experience with statistics, don't fret! This course starts with all of the concepts from the ground up. If you have experience with statistics, this course will allow you to associate each statistical concept with R Code to make you even more efficient. The first among these statistical tools is **central tendency**. 

As is evident from the words, tools of central tendency are concerned with the _centrality_ of the data, or the _middle value_ of the data. Now, the center or the middle can mean multiple different things as far as data is concerned. 
...
For this, we'll rely on a plot that is known as a **Histogram**. A Histogram is a nifty plot that groups data into ranges or "bins" and showcases the frequency of our data points within these ranges. Let's go ahead and visualize it!

```{r}
ggplot(swiss) + 
  geom_histogram(aes(y = Agriculture), 
                 bins = 30, 
                 fill="lightgray", 
                 color="black", 
                 alpha = 0.7) +
...
These graphs allow us to observe the distribution of the observations across the different levels in our variables. For instance, we can observe that for the `Agriculture`, observations between 60 - 70 tends to have the highest frequency. Or on the other hand, most of the observations in the `Education` variable are in the 0 - 10 area. 
...
## The Key Components of Central Tendency

### Mean 
At its core, the mean is a simple concept – it is what you get when you distribute the total equally among every entry in the data set. Or otherwise, when you have a summation of all of the observations in a data set, divided by the number of observations in that data set. Formulaically, 

$$
\overline{x} = \frac{1}{n}\sum_{i=0}^{n} x_i =  \frac{Sum~of~All~Data~Points}{Total~Number~of~Data~Points}
$$
Here, $\Sigma$ stands for summation, and $\overline{x}$ is what is used to represent the mean. While this may be enough for you to understand the concept, we can nuance this explanation a slight bit and make it more intuitive to interpret!

Let's imagine a scenario within the context of our `swiss` dataset. Say, every canton in Switzerland decided to unify and pool together their draftees as their resources, then distribute them equally based on their Education rates. The rate at which these resources would be distributed would be the **Mean Education Rate**. In R, the Mean is calculated quite simply through the `mean` function:
...
This allows us to notice that the **Mean Education Rate** across all of the Swiss cantions is **10.98%**. You can practice this yourself as well! Try to calculate the Mean Fertility Rate within the Swiss cantons in the code block below:

```{r}
#| eval: false
# Note: The first blank is supposed to be the function, and the sencond blank is supposed to be the variable
mean_fertility <- ...(swiss$...)
```
...
ggplot(swiss) + 
  geom_histogram(aes(y = Education), 
                 ...
                 color="black", 
                 alpha = 0.7) +
  geom_hline(aes(yintercept=mean_education), color="red", linetype="dashed", size=1) +
  labs(title = "Histogram of Education in Draftees", 
       x = "Frequency", 
       y = "% Education beyond Primary School for Draftees.") +
  scale_x_continuous(breaks = seq(min(swiss$Education), max(swiss$Education))) +
...
However, as with any tool, it is important to understand the appropriate use of the tool, as well as its limitations. While the mean is a wonderful tool in an economist's toolkit, one of its primary limitations is that it is severely affected by extreme values. 
...
First, let's calculate the original mean fertility rate:
...
The, let's introduce an extreme value. For the sake of illustration, we'll assign an unrealistically high fertility rate (e.g., 1000) to the first canton:
...
```{r}
ggplot() +
  geom_histogram(data = swiss, aes(x=Fertility), fill="blue", alpha=0.5, boundary = 0) +
  geom_vline(aes(xintercept=original_mean), color="blue", linetype="dashed", size=1) +
  geom_vline(data = swiss, aes(xintercept=extreme_mean), color="red", linetype="dashed", size=1) +
...
In this plot, the blue histogram represents the original swiss data set, while the red histogram represents the data set with the extreme value. The dashed lines indicate the mean of each data set. It becomes evident how the mean shifts due to just one extreme value, showcasing the sensitivity of the mean to outliers.
...
To calculate the Median, you ought to arrange data in ascending (or descending) order. Let $n$ be the number of data points. If $n$ is odd, then:
...
On the other hand, in R, computing the median is straightforward using the built-in median() function.
...
## Mode
The Mode simply signifies the value(s) that **appears most frequently** in a data set. This stands in contrast to other measures like the mean, which gives an average, or the median, which provides a midpoint. 

The beauty of the mode is its _versatility_. It's relevant for both numeric data sets and categorical data, such as strings or factors. This means that the Mode can be used to gauge whether a value, per say 5, appears with the greatest frequency in a data set, as well as if a category like "Female" or "University Graduate" appears with the greatest frequency.

A data set's relation with mode can be quite varied. It might not have a mode if no value repeats, be uni-modal if one value dominates in frequency, bi-modal if two values tie in their recurrence, or even multi-modal if multiple values share the highest frequency.

In R, we can calculate the mode without relying on external packages, since unlike the `mean` or the `median` fucntion, there is no `mode` function. Consider a function that first creates a frequency table of the data set in question. It then identifies the maximum frequency from this table. Using this frequency, it's possible to extract the modes, which are the values that appear with this maximum frequency. Here's how it might look:
