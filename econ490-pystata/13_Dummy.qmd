...
categories: [econ 490, pystata, dummy variable, interaction, regression, interpretation, coefficients]
format:
  html: default
  ipynb:
    jupyter:
...
You will remember dummy variables from when they were introduced in [Module 6](https://comet.arts.ubc.ca/docs/econ_490/econ490-stata/6_Creating_Variables.html). There we discussed both how to interpret and how to generate this type of variable. If you have any uncertainty about what dummy variables measure, please make sure you review that module.

Here we will discuss including qualitative variables as explanatory variables in a linear regression model.

Imagine that we want to include a new explanatory variable in our multivariate regression from [Module 11](https://comet.arts.ubc.ca/docs/econ_490/econ490-stata/11_Linear_Reg.html) that indicates whether an individual is identified as female. To do this we need to include a new dummy variable in our regression.
...
In [Module 6](https://comet.arts.ubc.ca/docs/econ_490/econ490-stata/6_Creating_Variables.html) we introduced the command `gen` (or `generate`). It is used to create new variables. Here, we are generating a new variable based on the values of the already existing variable _earnings_. 
...
We remember from [Module 11](https://comet.arts.ubc.ca/docs/econ_490/econ490-stata/11_Linear_Reg.html) that "_cons" is the constant $β_0$, and we know that here $β_0 = E[logearnings_{i}|female_{i}=0]$. Therefore, the results of this regression suggest that on average, males have log earnings of 10.8. We also know from the [Module 11](https://comet.arts.ubc.ca/docs/econ_490/econ490-stata/11_Linear_Reg.html) that
...
In this data set we also have a region variable that has 5 different regions. As in [Module 6](https://comet.arts.ubc.ca/docs/econ_490/econ490-stata/6_Creating_Variables.html), we can create dummies for each category using `tabulate`. 
...
Of course, we could also create a new `global` as was learned in [Module 4](https://comet.arts.ubc.ca/docs/econ_490/econ490-stata/4_Locals_and_Globals.html) that includes all of the dummy variables and includes that in the regression. Here is an example of what that would look like:
