...
categories: [econ 490, pystata, instrumental variable, endogeneity, relevance, exclusion, 2SLS, weak instrument]
format:
  html: default
  ipynb:
    jupyter: 
...
**Caution**: We can run 2SLS following the steps above, but when we want to do inference we need to be sure we're using the true residuals in the Structural equation $\hat{u}_i$. The built-in Stata command `ivregress` or `ivreg2` automatically give us the right residuals. </div>
